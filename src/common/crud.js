...
function update(model, populate = []) {
    return (req, res) => {
        ...
        return model.findByIdAndUpdate(req.params.id, req.body, { new: true }, errData(res)).populate(...populate);
    };
}

function remove(model) {
    return (req, res) => model.deleteOne({ id: req.params.id }, errData(res));
}
